...
root.innerHTML = `
  ...
  <main class="layout">
    ...
    <aside class="right panel"><div class="rule first"></div><label>ACTIVE ASSEMBLY</label><h2>STACKCHAN BODY</h2><p class="muted">M5STACK / K151</p><div class="stats"><span>WIDTH<strong>54.0 mm</strong></span><span>HEIGHT<strong>70.5 mm</strong></span><span>DEPTH<strong>61.5 mm</strong></span></div><div class="rule"></div><label>INSPECTION</label><div class="readout"><span>PARTS LOADED<strong id="partCount">0 / 10</strong></span><span>EXPLODE<strong id="explodeValue">0%</strong></span><span>MODEL SOURCE<strong>OFFICIAL STL</strong></span></div><div class="rule"></div><label>ASSEMBLY CONTROL</label><div class="control"><span>EXPLODE VIEW</span><input id="explode" type="range" min="0" max="100" value="0" /></div><button class="action" id="reset">RESET CAMERA <b>↺</b></button><a class="source" href="https://github.com/m5stack/M5_Hardware/tree/master/Products/K151_StackChan/Structures" target="_blank" rel="noreferrer">OPEN SOURCE RECORD ↗</a></aside>
  </main>`;
...
function assembleCoreS3(source){const input=source.attributes.position.array,bins=[[],[],[],[]],bounds=[[-Infinity,55],[55,115],[115,195],[195,Infinity]];for(let i=0;i<input.length;i+=9){const cx=(input[i]+input[i+3]+input[i+6])/3;const b=bounds.findIndex(([min,max])=>cx>=min&&cx<max);if(b>=0)bins[b].push(...input.slice(i,i+9));}const group=new THREE.Group();const colors=['#93aaa0','#718078','#a9b9b0','#c2d0c8'];bins.forEach((vertices,i)=>{if(!vertices.length)return;let min=Infinity,max=-Infinity;for(let n=0;n<vertices.length;n+=3){min=Math.min(min,vertices[n]);max=Math.max(max,vertices[n]);}const center=(min+max)/2;const array=new Float32Array(vertices);for(let n=0;n<array.length;n+=3)array[n]-=center;const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.BufferAttribute(array,3));g.computeVertexNormals();group.add(new THREE.Mesh(g,new THREE.MeshStandardMaterial({color:colors[i],metalness:.12,roughness:.7,transparent:true,opacity:.86})));});return group;}
