...
AtomS3R already supported SPIFFS-only config. Upstream now also falls back to SPIFFS on CoreS3/Core2 when SD is missing (same flat filenames). Our old `0001` patch is obsolete against current main:
