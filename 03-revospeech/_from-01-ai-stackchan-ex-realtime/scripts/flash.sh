#!/usr/bin/env bash
# Build + flash CoreS3 firmware and SPIFFS filesystem image.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
FW="$ROOT/upstream/firmware"
ENV_NAME="${PIO_ENV:-m5stack-cores3-local-bridge}"
...
echo "==> Applying local YAML configuration"
"$ROOT/scripts/apply-local-config.sh"
