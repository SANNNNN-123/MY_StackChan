      ...
      "{"
        ...
        "\"session\": {"
          ...
          "\"audio\": {"
            ...
            "}"
          "},"
          "\"instructions\": \"You are an AI robot named Stack-chan. Speak in Bahasa Melayu Malaysia with a natural Malaysian Malay accent. Use Malaysian vocabulary and pronunciation, not Indonesian Malay.\","
          "\"tools\":[]"
        "}"
      "}";
	...
	switch(type) {
		...
		case WStype_TEXT:
            ...
            if(msgType.equals("session.updated")){
                ...
                if(!isFuncCall){
#ifndef REALTIME_API_WITH_TTS
                    while (M5.Speaker.isPlaying() || uxQueueMessagesWaiting(p_this->audioReadyQueue) > 0) { vTaskDelay(1); }
                    M5.Speaker.end();
                    M5.Mic.begin();
                    exitMutexAudio();
