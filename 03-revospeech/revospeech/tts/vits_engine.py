...
# Standard Piper/espeak symbol order. Some exported Piper-compatible models
# omit this map from model.onnx.json even though the ONNX graph still expects
# the standard IDs.
_PIPER_SYMBOLS = (
    "_^$ !'(),-.:;?abcdefhijklmnopqrstuvwxyz"
    "æçðøħŋœǀǁǂǃɐɑɒɓɔɕɖɗɘəɚɛɜɞɟɠɡɢɣɤɥɦɧɨɪɫɬɭɮɯɰɱɲɳɴɵɶɸɹɺɻɽɾʀʁʂʃʄʈʉʊʋʌʍʎʏʐʑʒʔʕʘʙʛʜʝʟʡʢʲˈˌːˑ˞βθχᵻⱱ"
    "0123456789̧̪̯̩̃ʰˤε↓#\"↑̺̻gʦX̝̊ɝʷ"
 )


def _default_piper_phoneme_map() -> dict[str, list[int]]:
    return {symbol: [index] for index, symbol in enumerate(_PIPER_SYMBOLS)}
...
class VitsTTS(BaseTTS):
    ...
    def _ensure_repo(self) -> Path:
        ...
        configured_dir = os.environ.get("REVOSPEECH_VITS_MODEL_DIR")
        cache_dir = (
            Path(configured_dir).expanduser()
            if configured_dir
            else Path.home() / ".cache" / "revospeech" / self.manifest.name
        )
        self._models_dir = cache_dir

        if (cache_dir / "speakers.json").exists() or (
            cache_dir / "speakers" / "sarah" / "model.onnx"
        ).exists():
            return self._models_dir
    ...
    def _get_fallback_phoneme_map(self, models_dir: Path) -> dict[str, list[int]]:
        ...
        for spk in PRODUCTION_SPEAKERS:
            ...
            if raw:
                ...
                return self._fallback_phoneme_map
        self._fallback_phoneme_map = _default_piper_phoneme_map()
        return self._fallback_phoneme_map
